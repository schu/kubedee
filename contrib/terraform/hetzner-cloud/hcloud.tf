...
resource "hcloud_server" "test" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      # Create cluster
      "./bin/kubedee up --apiserver-extra-hostnames '${hcloud_server.test[count.index].name},${hcloud_server.test[count.index].ipv4_address}' --kubernetes-version '${var.kubernetes_version}' test",

      # Setup ingress
      "kubectl apply -f kubedee/manifests/ingress-nginx.yml",

      # Make ingress-nginx accessible
      "sudo iptables -t nat -A PREROUTING -p tcp -d ${hcloud_server.test[count.index].ipv4_address} --dport 80 -j DNAT --to-destination $(./bin/kubedee controller-ip test):80",
      "sudo iptables -t nat -A PREROUTING -p tcp -d ${hcloud_server.test[count.index].ipv4_address} --dport 443 -j DNAT --to-destination $(./bin/kubedee controller-ip test):443",

      # Make apiserver accessible
      "sudo iptables -t nat -A PREROUTING -p tcp -d ${hcloud_server.test[count.index].ipv4_address} --dport 6443 -j DNAT --to-destination $(./bin/kubedee controller-ip test):6443",
    ]
  ...
  provisioner "local-exec" {
    command = "scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ubuntu@${hcloud_server.test[count.index].ipv4_address}:~/.local/share/kubedee/clusters/test/kubeconfig/admin.kubeconfig . && sed -i -e 's|server:.*|server: https://${hcloud_server.test[count.index].ipv4_address}:6443|' admin.kubeconfig"
